...
public class ItemUseEvent {
    ...
    public void onInteractEvent(PlayerInteractEvent pEvent) {
        ...
        if (pEvent.action == Action.LEFT_CLICK_BLOCK || !pEvent.isCancelable()) return;

        // Ignore players in Creative-Mode
        if (pEvent.entityPlayer.capabilities.isCreativeMode && BlockLimiter.Config.DenyCreativeMode) return;
        ...
        if (pEvent.entityPlayer != null && pEvent.entityPlayer.getCurrentEquippedItem() != null) {
            ...
            for (ItemInfo tII : _mConfig.LimitedItems) {
                if (tII.isDenied(tID, pEvent.entityPlayer.dimension)) {
                    ...
                    {
                        if (BlockLimiter.Config.SFXOnItemDeny.length() > 0) pEvent.entityPlayer.worldObj
                                .playSoundAtEntity(pEvent.entityPlayer, BlockLimiter.Config.SFXOnItemDeny, 1F, 1F);

                        int tMsgIdx = _mRnd.nextInt(BlockLimiter.Config.RandomItemDenyMessages.length);
                        PlayerChatHelper.SendNotifyWarning(
                                pEvent.entityPlayer,
                                BlockLimiter.Config.RandomItemDenyMessages[tMsgIdx]);
                    } catch (Exception e) {
