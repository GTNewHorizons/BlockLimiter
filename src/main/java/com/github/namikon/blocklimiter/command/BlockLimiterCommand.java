    ...
    public void processCommand(ICommandSender pCmdSender, String[] pArgs) {
        ...
        if (pArgs[0].equalsIgnoreCase("reload")) {
            if (!isOpOrAdmin(pCmdSender)) {
                ...
                if (!BlockLimiter.Config.Reload()) PlayerChatHelper.SendError(
                        pCmdSender,
                        "Blocks could not be reloaded properly. Please check the console output and fix the config file");
                else {
                    PlayerChatHelper.SendInfo(pCmdSender, "Blocklimiter config reloaded. Now monitoring:");
                    PlayerChatHelper.SendInfo(
                            pCmdSender,
                            String.format(" - %d Block(s)", BlockLimiter.Config.LimitedBlocks.size()));
                    PlayerChatHelper.SendInfo(
                            pCmdSender,
                            String.format(" - %d Item(s)", BlockLimiter.Config.LimitedItems.size()));
                }
            ...
            if (BlockLimiter.Config.LimitedBlocks.size() == 0) {
                ...
                PlayerChatHelper
                        .SendInfo(pCmdSender, String.format("The following Blocks are monitored by BlockLimiter:"));
            ...
            if (BlockLimiter.Config.LimitedItems.size() == 0) {
                ...
                PlayerChatHelper
                        .SendInfo(pCmdSender, String.format("The following Items are monitored by BlockLimiter:"));
        ...
        else PlayerChatHelper.SendError(pCmdSender, "Usage: /blimit reload|blockinfo|iteminfo");
